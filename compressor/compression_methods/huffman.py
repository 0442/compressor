...
class Huffman(CompressionMethod):
  ...
  def _encode_text(self, text: TextIO, huffman_codes: dict[str, bytearray]) -> bytearray:
    """
  ...
  def _decode_tree(self, tree_str: str) -> HuffmanTreeNode | None:
    ...
    def decode_tree(tree_str: str, depth: int = 0) -> tuple[HuffmanTreeNode | None, str]:
      if not tree_str:
  ...
  def _read_headers(self, bin_in: BinaryIO) -> tuple[int, int]:
    ...
    if (padding_len < 0 or tree_len < 0 or
      padding_len >= 8 or tree_len > len(bin_in.read())):
      raise ValueError(
